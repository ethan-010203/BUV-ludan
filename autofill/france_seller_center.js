...
export default {
  ...
  async buildPlan({ modulesData, foundFiles, aiData, utils }) {
    ...
    // 营业执照上传框 accept=".jpg,.jpeg,.png" —— 不转 PDF，直接发第一页 JPEG
    const licensePayload = await fileToPayload(license, { keepImage: true });
    ...
    const plan = [
      ...
      // 拼音名拆成姓 / 名两段（法国页面示例文案是 Yang / Xingying）
